...
class JsonToCV {
	...
	_validateThemePath(themePath) {
		if (!exists(path.join(themePath, this.names.templateFile))) {
			throw new Error(`'${this.opts.theme}' is not a valid theme`);
		}
		return themePath;
	}
	...
	_getThemePath() {
		let themePath;
		let localTheme = path.join(this.opts.dir, this.opts.theme);
		let builtInTheme = path.join(__dirname, "./../themes", this.opts.theme);
		...
		throw new Error(`no theme found with '${this.opts.theme}' name`);
	}
	...
	async cloneTheme() {
		if (this.themeOrigin === "local") {
			throw new Error(`${this.opts.theme} is not a built-in theme`);
		}
		let themeDir = path.join(this.paths.dir, this.opts.theme);
